      ...
      <div class="row"  style="margin-top:1%">
        <div class="col l3 hide-on-med-and-down">
          <div class="card-panel green darken-2">
            <div class="row">
              <a class="waves-effect waves-light btn green" href="<?php echo site_url('Portofolio/newContent'); ?>" style="width:100%"><i class="material-icons right">add</i>New Content</a>
            </div>
          </div>
        </div>
                ...
                <tbody class="white-text">
                  <?php foreach($dataCV as $row): ?>
                    <tr>
                      ...
                      <td><?php echo $row->tanggalDibikinport ?></td>
                      <td><?php echo $row->judulKontenPort ?></td>
                      <td><?php echo $row->Konten ?></td>
                      <td><?php echo $row->published ?></td>
                      <td>
                        <div class="row">
                          <div class="col l4 s6">
                            <a title = "Hapus Content" onclick="confirmDelete()" href="<?php echo site_url('Portofolio/deleteContent')."/".$row->IDport; ?>" class="material-icons white-text">delete</a>
                          </div>
                          <div class="col l4 s6">
                            <a title = "Update Content" href="<?php echo site_url('Portofolio/updateContent')."/".$row->IDport; ?>" class="material-icons white-text">description</a>
                          </div>
                          <div class="col l4 s6">
                            <?php if($row->published==FALSE) { ?>
                            <a title = "Publish Content" href="<?php echo site_url('Portofolio/publishContent')."/".$row->IDport; ?>" class="material-icons white-text">cloud</a>
                            <?php } else { ?>
                            <a title = "Unblish Content" href="<?php echo site_url('Portofolio/unpulishContent')."/".$row->IDport; ?>" class="material-icons white-text">clear</a>
                            <?php } ?>
                          </div>
                        </div>
                      </td>
